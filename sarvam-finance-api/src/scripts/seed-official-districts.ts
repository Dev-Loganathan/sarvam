...
export const INDIA_DISTRICTS_JSON = {
  states: [
    {
...
async function seedOfficialDistricts() {
  console.log('🚀 Seeding exact official India Districts dataset into Supabase...');
  ...
  try {
    ...
    for (const stateItem of INDIA_DISTRICTS_JSON.states) {
      const stateName = stateItem.state.trim();
      const cleanStateName = stateName.replace(/'/g, "''");
      const stateCode = stateName.substring(0, 3).toUpperCase().replace(/[^A-Z]/g, '');

      // Upsert State
      const existing: any[] = await db.$queryRawUnsafe(`
        SELECT id FROM "StateMaster"
        WHERE "countryId" = '${activeCountryId}'
        AND (LOWER(name) = LOWER('${cleanStateName}') OR LOWER(code) = LOWER('${stateCode}'))
        LIMIT 1;
      `);
      ...
      if (!stateDbId) {
        const stateResult: any[] = await db.$queryRawUnsafe(`
          ...
          VALUES (gen_random_uuid()::text, '${stateCode}', '${cleanStateName}', '${activeCountryId}', NOW())
          RETURNING id;
        `);
        stateDbId = stateResult[0]?.id;
      } else {
        // Ensure state name matches official format
        await db.$executeRawUnsafe(`
          UPDATE "StateMaster" SET name = '${cleanStateName}' WHERE id = '${stateDbId}';
        `);
      }

      totalStates++;

      if (!stateDbId || !stateItem.districts || stateItem.districts.length === 0) continue;

      const uniqueDistricts = Array.from(new Set(stateItem.districts.map((d) => d.trim()))).filter(Boolean);
      ...
      for (let i = 0; i < uniqueDistricts.length; i += BATCH_SIZE) {
        ...
        // Insert into CityMaster as fallback
        await db.$executeRawUnsafe(`
    ...
    console.log(`\n🎉 SUCCESS: Imported exact official India Districts dataset into Supabase!`);
    ...
    console.error('❌ Error seeding official districts:', error);
    process.exit(1);
  }
}

seedOfficialDistricts();
